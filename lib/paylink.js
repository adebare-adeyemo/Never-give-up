 ...
 *
 * The invoice email needs a link that still works days later, but a Stripe
 * Checkout Session expires within 24 hours. So the email links to our own
 * /pay/<token> route, which creates a fresh Checkout Session at the moment the
 * customer clicks.
 *
...
function secret() {
  /*
   * Falls back to the Stripe secret key so there is one fewer required
   * variable: payment links only exist when Stripe is configured anyway. Set
   * BOOKING_LINK_SECRET explicitly to rotate links without rotating Stripe keys.
   */
  const value = process.env.BOOKING_LINK_SECRET || process.env.STRIPE_SECRET_KEY;
...
/** Short human-readable reference shown on the invoice and in Stripe. */
