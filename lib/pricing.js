 ...
 *
 * Every figure here is taken directly from the published price list on
 * /pricing. Nothing is estimated or invented: where the price list says
 * "Custom Quote", the service is marked `quoteOnly` and no total is calculated
 * — those bookings are taken as enquiries and priced by hand.
 *
...
/** 60% of the booking value, charged for cancellations inside the notice period. */
export function cancellationFeePence(totalPence) {
  return Math.round(totalPence * CANCELLATION.feeRate);
}
