 ...
 */
export function invoiceHtml({ reference, data, quote, payUrl }) {
  const total = formatPence(quote.totalPence);
  const cancellationFee = formatPence(cancellationFeePence(quote.totalPence));

  const paymentExplainer = `Payment of ${total} is taken when you complete the form below. If the work does not go ahead, you are refunded in full.`;
        ...
        <tr>
          <td style="padding:28px 32px 0;">
            ...
            <p style="margin:14px 0 0;font-family:${FONT};font-size:13px;line-height:20px;color:${C.subtle};text-align:center;">
              Payments are handled by takepayments. We never see or store your card details.
            </p>
          </td>
        </tr>
...
export function invoiceText({ reference, data, quote, payUrl }) {
  const total = formatPence(quote.totalPence);
...
HOW PAYMENT WORKS
Payment of ${total} is taken when you complete the form. If the work does not
go ahead, you are refunded in full.
Payments are handled by takepayments. We never see or store your card details.
