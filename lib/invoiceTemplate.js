...
import { formatPence, CANCELLATION } from '@/lib/pricing';
...
export function invoiceHtml({ reference, data, quote, payUrl, holdsFunds }) {
  ...
  const cancellationFee = formatPence(Math.round(quote.totalPence * CANCELLATION.feeRate));
        ...
        <tr>
          <td style="padding:16px 32px 0;">
            <p style="margin:0;font-family:${FONT};font-size:13px;line-height:21px;color:${C.subtle};">
              ...
              ${Math.round(CANCELLATION.feeRate * 100)}% applies — ${cancellationFee} on this booking —
...
export function invoiceText({ reference, data, quote, payUrl, holdsFunds }) {
  const total = formatPence(quote.totalPence);
  const fee = formatPence(Math.round(quote.totalPence * CANCELLATION.feeRate));
...
CANCELLATIONS
Cancel more than ${CANCELLATION.noticeHours} hours before your clean and you pay nothing.
Cancel with less than ${CANCELLATION.noticeHours} hours' notice and a ${Math.round(CANCELLATION.feeRate * 100)}% cancellation
fee applies — ${fee} on this booking. Full terms: ${SITE.url}/terms
