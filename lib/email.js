...
export async function sendBookingNotification(data, { note = '' } = {}) {
 ...
 */
export async function sendCustomerAcknowledgement(data, { note = '' } = {}) {
  const transporter = createTransport();
 ...
 */
export async function sendBookingInvoice({ data, quote, reference, payUrl }) {
  const transporter = createTransport();
  const { from } = addresses();
  ...
  await transporter.sendMail({
    ...
    html: invoiceHtml({ reference, data, quote, payUrl }),
    text: invoiceText({ reference, data, quote, payUrl }),
  });
}

/**
 * Tells the business a payment has cleared, and states the refund amounts so
 * nobody has to work out the cancellation fee under pressure.
 */
export async function sendPaymentNotice({
  ...
  const netRefund = formatPence(amountPence - cancellationFeePence(amountPence));
  ...
  const actions = `<div style="margin-top:18px;padding:14px 16px;background:#f0fdfa;border-left:4px solid #0f766e;">
         ...
         <p style="margin:0;">• Cancelled with less than ${CANCELLATION.noticeHours}h notice &rarr; refund <strong>${escapeHtml(netRefund)}</strong>, keeping the ${escapeHtml(lateFee)} cancellation fee</p>
       </div>`;

  const heading = `Payment received — ${total}`;
  ...
  await transporter.sendMail({
    ...
    text:
      ...
      `Reference: ${reference}\nCustomer: ${name}\nEmail: ${email}\nService: ${service}\n` +
      `Cleaning date: ${cleaningDate}\nPaid: ${total}\nGateway reference: ${gatewayReference}\n\n` +
      `In the takepayments portal:\n` +
      `  Clean did not go ahead       -> Refund ${total} in full\n` +
      `  Cancelled <${CANCELLATION.noticeHours}h notice        -> Refund ${netRefund}, keep ${lateFee}\n`,
  });
}
