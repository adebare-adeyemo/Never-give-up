 ...
 * has to work out 60% of a booking under pressure.
 */
export async function sendPaymentNotice({
  ...
  const actions = held
    ? `<div style="margin-top:18px;padding:14px 16px;background:#f0fdfa;border-left:4px solid #0f766e;">
         ...
         <p style="margin:0 0 4px;">• Cancelled with less than ${CANCELLATION.noticeHours}h notice &rarr; <strong>Capture ${escapeHtml(lateFee)}</strong> (60%)</p>
    ...
    : `<div style="margin-top:18px;padding:14px 16px;background:#f0fdfa;border-left:4px solid #0f766e;">
         <p style="margin:0 0 8px;"><strong>This payment has been taken.</strong> If the booking changes, refund in the Stripe Dashboard:</p>
